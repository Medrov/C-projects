...
/*
Перед тем как читать код, нужно понять, что честно говоря я под устал к декабрю и задолбался писать идеальный код.
Так что, если вы найдете какие-то баги, а вы скорее всего найдете, то не печальтесь. А вот самая важная проблема это память)
Я за ней не слежу, потому что не хочу заниматься клоунадой. Если вам она так важна можете чекать через valgrind на Комаровском дерьме.
Удачи, я в таких делах не хочу участвовать. Я пишу и сдаю код в VS Code)
Как-то так, ребята)
*/
...
void outputMenu(int **matrix, int length)
{
    printf(
        ...
        "       Outputting Menu\n"
        "------------------------------\n");
    printf("1. Запись исходных данных в стандартный поток вывода («на экран»)\n");
